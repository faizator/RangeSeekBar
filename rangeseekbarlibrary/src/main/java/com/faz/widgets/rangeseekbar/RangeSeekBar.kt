package com.faz.widgets.rangeseekbar
...
class RangeSeekBar @JvmOverloads constructor(
    ...
    init {
        ...
        if (attrs != null) {
            ...
            inactiveColor = arr.getColor(R.styleable.RangeSeekBar_inactiveColor, DEFAULT_INACTIVE_COLOR)
            textColor = arr.getColor(R.styleable.RangeSeekBar_textColor, DEFAULT_TEXT_COLOR)
            arr.recycle()
        }

        thumbImage = (lDrawable ?: context.resources.getDrawable(DEFAULT_DRAWABLE_RES)).toBitmap()
        thumbHalfWidth = 0.5f * thumbImage.width
        thumbHalfHeight = 0.5f * thumbImage.height
        lineHalfHeight = 0.5f * 0.25f * thumbImage.height.toFloat()
        xOffset = thumbHalfWidth.toInt()

        textSize = if (lTextSizeInPx == 0f) dpToPx(context, DEFAULT_TEXT_SIZE_IN_SP) else lTextSizeInPx
    }

    private fun dpToPx(context: Context, dp: Float): Float = TypedValue.applyDimension(
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        canvas.drawRect(xOffset.toFloat(),
                thumbHalfHeight - lineHalfHeight,
        ...
        canvas.drawRect(valueToScreen(selectedMinValue).toFloat(),
                thumbHalfHeight - lineHalfHeight,
                valueToScreen(selectedMaxValue).toFloat(),
                thumbHalfHeight + lineHalfHeight,
                paint)
        ...
        // draw the text
        paint.textSize = textSize
        paint.flags = FAKE_BOLD_TEXT_FLAG
        ...
        if (maxTextX + maxTextWidth > WIDTH) {
            maxTextX = (WIDTH - maxTextWidth).toDouble()
        }
        ...
        canvas.drawText(minText,
                minTextX.toFloat(),
                2 * textSize + thumbHalfHeight + thumbHalfHeight + minTextYOffset,
                paint)

        canvas.drawText(maxText,
                maxTextX.toFloat(),
                2 * textSize + thumbHalfHeight + thumbHalfHeight + maxTextYOffset,
                paint)
    }
    ...
    private fun isInThumbRange(touchX: Float, normalizedThumbValue: Double): Boolean {
        return Math.abs(touchX - valueToScreen(normalizedThumbValue)) <= 2 * thumbHalfWidth
    }
    ...
    private fun screenToValue(screenCoordX: Float): Double {
        ...
        val result = (screenCoordX - xOffset) / (WIDTH - 2 * xOffset)
    ...
    companion object {
        ...
        val DEFAULT_DRAWABLE_RES = R.drawable.range_seek_thumb
